...
@bot.event
async def on_ready():
    print(f'arXiv Bot logged in as {bot.user}')

@bot.command(name='query')
async def query(ctx, *args):
    """Replicates sample/bot.py: Search arXiv and summarize 10 random papers."""
    if not args:
    ...
    try:
        ...
        num_papers = min(10, len(papers))
        results = random.sample(papers, k=num_papers)

        for i, result in enumerate(results):
            print(f"Processing {i+1}/{num_papers}")
            await ctx.send(f"Processing result {i+1}/{num_papers}")

            try:
                summary = await get_summary(result)
                message = f"**{query_arg}の論文です！ {i+1}/{num_papers}**\n"
                message += f"発行日: {result['published']}\n{result['url']}\n\n{summary}"
